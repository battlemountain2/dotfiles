...
Item {
  ...
  function toggleControlCenterAt(x) {
    var item = findPanelWidget("bry.control-center")
    if (item) {
      if (typeof item.toggleAtCursor === "function") {
        item.toggleAtCursor(x)
        return true
        ...
        MouseArea {
          ...
          onClicked: function(mouse) {
            if (mouse.button === Qt.RightButton) {
              ...
              root.toggleControlCenterAt(scenePoint.x)
              mouse.accepted = true
            }
          }
        }
